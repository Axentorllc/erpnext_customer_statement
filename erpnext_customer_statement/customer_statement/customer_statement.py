...
from frappe.utils.pdf import get_pdf, _
...
@frappe.whitelist()
def get_report_pdf(customer, company, from_date, to_date, consolidated=True, download=True):
	...
	for i in res:
		if i.get('voucher_type') == 'Journal Entry':
			i.user_remark = frappe.db.get_value('Journal Entry', i.voucher_no, 'user_remark')
	...
	html = frappe.render_template(base_template_path, {"body": html, "css": get_print_style()})
